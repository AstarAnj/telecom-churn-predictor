...
def load_data() -> pd.DataFrame:
    """Load churn data from the manually downloaded local CSV file."""
    if not os.path.exists(DATA_PATH):
        raise FileNotFoundError(
            f"CSV file not found at: {DATA_PATH}. "
            "Please place your downloaded Kaggle CSV in the data folder."
        )
...
def main():
    ...
    with tabs[0]:
        ...
        with st.expander("🔍 View raw data sample", expanded=False):
            st.dataframe(raw_df.head(100), use_container_width="stretch")
        ...
        with col1:
            ...
            for i, v in enumerate(counts.values):
                ax.text(i, v + 40, f"{v:,}", ha="center", fontweight="bold", fontsize=7)
            ax.set_ylabel("Customers", fontsize=7)
            ax.set_title("Churn vs Retained", fontsize=10)
            sns.despine()
            plt.tight_layout()
        ...
        ax.set_xlabel("Churn Rate (%)", fontsize=7)
        ax.set_title("Month-to-month contracts churn the most", fontsize=10)
        sns.despine()
        plt.tight_layout()
    ...
    with tabs[1]:
        ...
        with st.expander("🔍 View engineered features sample"):
            ...
            st.dataframe(df[eng_cols].head(50), use_container_width="stretch")
    ...
    with tabs[2]:
        ...
        with col1:
            ...
            st.dataframe(res_no, use_container_width="stretch", hide_index=True)

        with col2:
            st.subheader("With SMOTE")
            res_sm = evaluate_models(models_smote, X_test, y_test)
            st.dataframe(res_sm, use_container_width="stretch", hide_index=True)
